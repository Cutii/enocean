...
// Do not make this copy, it's an API footgun.
...
impl CRC8 {
    pub fn extend(mut self, bytes: &[u8]) -> Self {
        for byte in bytes {
            self.state = CRC_TABLE[(self.state ^ byte) as usize]
        }
        self
    }

    pub fn ok_or<E>(self, e: E) -> Result<(), E> {
        if self.state == 0 { Ok(()) } else { Err(e) }
    }
}
